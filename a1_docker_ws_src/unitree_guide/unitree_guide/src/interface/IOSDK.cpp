...
*/
	...
	for(int m(0); m < 12; ++m){
		_lowCmd.motorCmd[m].Kp = _lowCmd.motorCmd[m].Kp * pd_count / 2500;
	}
	
	if(walk_freq <= 1.7){
	 _lowCmd.motorCmd[0].tau = _lowCmd.motorCmd[0].tau * 0.9;
	 _lowCmd.motorCmd[3].tau = _lowCmd.motorCmd[3].tau * 0.9;
	 _lowCmd.motorCmd[6].tau = _lowCmd.motorCmd[6].tau * 0.9;
	 _lowCmd.motorCmd[9].tau = _lowCmd.motorCmd[9].tau * 0.9;
	 }
	 if(walk_freq >= 1.7){
	 _lowCmd.motorCmd[4].tau = _lowCmd.motorCmd[4].tau * 1.2;
	 }
    ...
    pub_nikukyu.data = _lowState.footForce[0];
    nikukyu.publish(pub_nikukyu);
